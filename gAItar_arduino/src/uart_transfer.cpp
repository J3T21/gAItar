...
const char* findFileRTOS(const String& title, const String& artist, const String& genre) {
    ...
    static char directoryPathBuffer[64];
    static char fullFilePathBuffer[128];
    static char nameBuffer[64];
    static char cleanTitleBuffer[128];  // Buffer for cleaned title
    
    // Use snprintf instead of String concatenation
    snprintf(directoryPathBuffer, sizeof(directoryPathBuffer), "/%s/%s", 
             genre.c_str(), artist.c_str());
    ...
    if (!directory.open(directoryPathBuffer)) {
        Serial.print("Failed to open directory: ");
        Serial.println(directoryPathBuffer);
    ...
    while (entry.openNext(&directory, O_RDONLY)) {
        ...
        // Use snprintf instead of String concatenation
        snprintf(fullFilePathBuffer, sizeof(fullFilePathBuffer), "%s/%s", 
                 directoryPathBuffer, nameBuffer);
        
        File file = sd.open(fullFilePathBuffer, FILE_READ);
        if (!file) {
            entry.close();
            continue;
        }
        ...
        if (error) {
            Serial.print("JSON parse error in file ");
        ...
        const char* fileTitle = doc["title"];
        
        if (fileTitle != nullptr) {
            // Manually clean the title by removing \r and \n characters
            int j = 0;
            for (int i = 0; fileTitle[i] != '\0' && j < sizeof(cleanTitleBuffer) - 1; i++) {
                if (fileTitle[i] != '\r' && fileTitle[i] != '\n') {
                    cleanTitleBuffer[j++] = fileTitle[i];
                }
            }
            cleanTitleBuffer[j] = '\0'; // Null terminate
            
            // Debug output
            Serial.print("Comparing cleaned title: '");
            Serial.print(cleanTitleBuffer);
            Serial.print("' with search title: '");
            Serial.print(title.c_str());
            Serial.println("'");
            
            if (strcmp(cleanTitleBuffer, title.c_str()) == 0) {
                // Found match
...
void fileReceiverRTOS_new(Uart &fileUart){
    ...
    auto resetState = [&]() {
        if (xSemaphoreTake(sdSemaphore, portMAX_DELAY)) {  // Use portMAX_DELAY for cleanup
            if (file) {
                file.close();
            }
            xSemaphoreGive(sdSemaphore);
    ...
    switch (state){
        ...
        case OPEN_FILE:
            ...
            // Use portMAX_DELAY for file opening - this is critical
            if (xSemaphoreTake(sdSemaphore, portMAX_DELAY)){
                file = sd.open(filePath.c_str(), FILE_WRITE);
                if(file){
                    lastByteTime = millis();
        ...
        case PARSE_CHUNK_HEADER:
            if (fileUart.available()){
                ...
                if (chunkHeader.startsWith("START:")) {
                    state = PARSE_HEADER;
                    break;
                }
        ...
        case READ_CHUNK:{
            ...
            if (bytesAccumulated >= chunkSize){
                ...
                // Use portMAX_DELAY for write operations - this is the critical fix
                if (xSemaphoreTake(sdSemaphore, portMAX_DELAY)){
                    size_t written = file.write(buffer, chunkSize);
                    file.flush();
                    xSemaphoreGive(sdSemaphore);
        ...
        case DONE:
            // Use portMAX_DELAY for file closing
            if (xSemaphoreTake(sdSemaphore, portMAX_DELAY)){
                if (file) file.close();
                xSemaphoreGive(sdSemaphore);
            }
            
            Serial.printf("Transfer complete: %s (%u bytes)\n", filePath.c_str(), receivedBytes);
